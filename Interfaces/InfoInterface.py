...
- 文件信息和预设的读取和保存功能现在由Java后端实现

## 优化内容

- 删除预设时会弹出对话框确认操作
- 若软件启动时Java路径不正确，则直到下次程序启动前不会保存任何数据，防止先前保存的数据被意外清除

## 修复BUG

- 修复添加文件时会多次保存相同内容的BUG
- 修复控制台输出窗口自动滚动无法关闭的BUG
...
- 修复点击命令行界面的输出文本框后下次输出内容会错位的BUG

## 优化内容

- 重定向文件时将自动定位到旧路径所指的文件夹
